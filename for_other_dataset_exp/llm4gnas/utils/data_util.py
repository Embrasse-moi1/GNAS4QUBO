...
base_path = "/home/chenyz/github/LLM4GNAS/dataset/"
...
def load_data(dataset_name, task_name):
    if task_name == "NodeClassification":
        return load_nc_data(dataset_name)
    elif task_name == "LinkPredict":
        return load_link_prediction_data(dataset_name)
def load_nc_data(dataset="acm", self_loop=True):
    dataset = dataset.lower()
    if dataset.lower() == "acm":
        return load_acm_raw(self_loop=self_loop, train_size=400, val_size=400)
    elif dataset.lower() == "dblp":
        return load_DBLP(prefix=base_path + "data/dblp/DBLP_processed/", self_loop=self_loop, train_size=400, val_size=400)
...
def load_imdb(self_loop=False):
    ...
    if os.path.exists(dgl_graph_file):
        ...
        with open(base_path+'/data/IMDB/node_features.pkl', 'rb') as f:
                node_features = pickle.load(f)
        with open(base_path+'/data/IMDB/edges.pkl', 'rb') as f:
                edges = pickle.load(f)
        with open(base_path+'/data/IMDB/labels.pkl', 'rb') as f:
...
def load_link_prediction_data(dataset="Yelp", self_loop=True, path=base_path +"data/", force=False):
    dataset = dataset.lower()
