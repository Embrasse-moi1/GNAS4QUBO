...
from llm4gnas.contrib.nas_method.arch2vec_model.layers import GraphConvolution
from llm4gnas.contrib.nas_method.arch2vec_model.utils.utils import preprocessing, normalize_adj
import time
from llm4gnas.contrib.nas_method.arch2vec_model.gin.models.mlp import MLP
import logging
...
class DecoderNN(object):
    def __init__(self, model, ops, adj, cfg):
        logging.info('Initializing NN decoder')
        t_s = time.time()
        self.model = model
        self.ops = ops
        self.adj = adj
        ...
        logging.info(f'Using {time.time()-t_s} seconds to initialize NN decoder')
